...
ExifTool does not need to be installed to run. However, installation makes it available to all users by placing exiftool files in the correct directories such as the main executable to ` /usr/bin/ `, documentation to `/usr/share/man/ ` and API libraries to ` /usr/lib64/ ` (see [Chapter 2, Section: Directory: /usr/](02-basic.md#directory-usr)).
